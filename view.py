...
def show_contacts(book: dict[int: dict[str, str]], message):
    if book:
        ...
        for index, contact in book.items():
            print(
                f'{index:>3}. {contact.get("name"):<{size_name}} '
...
def add_changes():
    result = {}
    for key, value in text.changes_contact.items():
...
def show_del(book: dict[int: dict[str, str]], message):
    if book:
        ...
        for index, contact in book.items():
            print(
                f'Контакт {index:>3}. {contact.get("name"):<{size_name}} {contact.get("phone"):<{size_phone}} '
                f'{contact.get("comment"):<{size_comment}} был удален')
        print('=' * (size_name + size_phone + size_comment + 7) + '\n')
    else:
        print_message(message)
